...
class BaseIsolatedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.param_to_id = {param: { val : i for i, val in enumerate(vals) if val != "-1"} \
            for param, vals in self.params.items() }

        self.id_to_param = {param: { i : val for i, val in enumerate(vals) if val != "-1"} \
            for param, vals in self.params.items() }

        for p in self.params.keys():
            self.param_to_id[p]['-1'] = -1
            self.id_to_param[p][-1] = '-1'
    ...
    def collate_fn(batch_list):
        ...
        return dict(frames=frames, labels=labels, params=params, files=[x["file"] for x in batch_list], dataset_names=[x["dataset_name"] for x in batch_list])
