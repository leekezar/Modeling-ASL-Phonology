...
class InferenceModel(pl.LightningModule):
    ...
    def test_inference(self):
        ...
        for batch in dataloader:
            ...
            # each value is a probability distribution for each item in the batch
            y_hat_params = { p : v.cpu() for p,v in y_hat[1].items() }

            
            for item in y_hat_gloss:
                all_preds = { dataloader.dataset.id_to_gloss[i] : conf for i,conf in item }

            class_indices = torch.argmax(y_hat_gloss, dim=-1)

            # each value is the index corresponding to the most probable class (param val)
            param_indices = { p : torch.argmax(v, dim=-1) for p,v in y_hat_params.items() }
            ...
            for i, pred_index in enumerate(class_indices):
                ...
                print(f"True: {filename}\tPredicted: {label}")
                
                row = { "true":filename.split("/")[-1].replace(".pkl",""), "preds": all_preds }
                
                assert 1 == 2

                for p, param_index in param_indices.items():
                    param_label = dataloader.dataset.id_to_param[p][int(param_index[i])]
                    param_conf = float(y_hat_params[p][i][param_index[i]])
                    
                    print(f"\t{p}: {param_label} ({param_conf:.2f})")
                    row[p] = { "pred" : param_label, "conf" : param_conf }
        ...
        json.dump(results, open(self.cfg.test_pipeline.dataset.results,"w+"), indent=4)
